...
if dev==False:
        ...
        if cmd=="help":
            ...
            def newtask():
                ...
                if but1['text'] == '开始' or but1['text'] == '继续':  # 根据按钮的文本来判断是否开启循环
                    if but1['text'] == '开始':
                        fo = open("计时器时间记忆.txt", "a+")  # 开始时清楚上一次记录的内容
                    else:
                        fo = open("计时器时间记忆.txt", "a+")  # 追加暂停时的时间
                    but1['text'] = '暂停'
                    isloop = True
            ...
            for i in range(number):
                ...
                one = chinese_list[0:7]
                two = chinese_list[7:14]
                three= chinese_list[14:21]
                four = chinese_list[21:28]
                title = chinese_list[28:32]
                a1 = "".join(one)
                b1 = "".join(two)
                c1 = "".join(three)
                d1 = "".join(four)
    ...
    {3},
    {4}。
            """.format(t1, a1, b1, c1, d1)
                print(say)
                file=open("终极吟诗.txt","a+",encoding="utf-8")
                file.write(say+"\n")
            print("已将吟诗存储到“终极吟诗.txt”文件中")
            file.close()

        elif cmd=="加密机":
            test=[]
            last=[]
            string = input("输入你要加密的内容:")
            for i in string:
                test.append(chr(ord(i)+10))
            for i in range(len(test)//2):
                test[i],test[-i]=test[-i],test[i]
            last=test
            print("加密成功")
            time.sleep(0.5)
            print("加密后的内容是:{0}".format("".join(last)))
            print("请使用解密机解密")
        elif cmd=="解密机":
            last = []
            string = input("输入你要解密的内容:")
            for i in string:
                last.append(chr(ord(i) - 10))
            for i in range(len(last) // 2):
                last[i], last[-i] = last[-i], last[i]
            time.sleep(1)
            print("解密成功")
            time.sleep(0.5)
            print("解密后的内容是:{}".format("".join(last)))
        elif cmd=="石头剪刀布":
            choices = ["石头", "布", "剪刀"]
            ...
            player_score = 0
            while True:
                player = input("石头,布 or 剪刀(输入'结束'结束)").capitalize()
                # 判断游戏者和电脑的选择
                if player == computer:
                    print("平局!")
                elif player == "石头":
                    ...
                    if computer == "石头":
                        ...
                        print("你赢了✌️!", player, "cut", computer)
                        player_score += 1
                elif player == '结束':
                    print("——————最终得分——————")
                    print(f"电脑:{cpu_score}")
            ...
            if __name__ == "__main__":
                ...
                cv1.place(x=0, y=0)
                w = Label(root, text="你每走一步,就可能寄！小心亿点！", font=("楷体", 12))
                w.place(x=60, y=385)
                button_restart = Button(root, text="🆕", font=('楷体', 15), bg="#AAAAAA", fg="blue", command=restart)
            ...
            try:
                password_len=int(input("输入密码长度:"))
            except Exception:
                print("输入错误,返回🔙")
            password_word="abcdefghijklmnopqrstuvwxyz1234567890_!@#$%^&*"
            password="".join(random.sample(password_word,password_len))
            print("生成后的密码是:{0}".format(password))
        elif cmd=="文件管理" and dev==True:
            file_out = """注意⚠️：
    1.这是开发者模式，可能会出现各种奇怪的问题
    2.目前文件管理只能使用txt文件
    3.不要瞎输入，出来问题自己承担"""
            print(file_out)
            time.sleep(1)
            mode = ["读", "覆盖写", "读+覆盖写", "追加", "读+追加", "创建"]
            file_mode = input("选择模式[读，覆盖写，读+覆盖写，追加，读+追加，创建]：")
            while file_mode not in mode:
                print("模式输入错误")
                time.sleep(0.3)
                file_mode = input("选择模式[读，覆盖写，读+覆盖写，追加，读+追加，创建]：")
            file_route = input("输入文件路径(绝对路径)：").replace("\\", "\\\\")
            while "txt" not in file_route:
                print("检测到不是txt文件,重新输入")
                time.sleep(0.3)
                file_route = input("输入文件路径(绝对路径)：").replace("\\", "\\\\")
            try:
                if file_mode == "读":
                    ...
                    file_write = input("输入要覆盖写的内容(换行\\n,Tab\\t)：")
                    file.write(file_write)
                    print("写入完毕")
                elif file_mode == "读+覆盖写":
                    file = open(file_route, "w+")
                    file_read = file.read()
                    print("文件编辑前读取：\n{0}".format(file_read))
                    time.sleep(0.5)
                ...
                print("寄，文件管理beta出现了一些《小问题》")
                time.sleep(0.5)
                print("问题是:{0}".format(e))
                print("可以将输入输出数据发送到silicon_siloud@outlook.com邮箱反馈,记得留下你的联系方式(邮箱orQQor微信)")
            else:
                print("不容易啊，竟然没有问题！鼓掌！")
            finally:
                file.close()
                print("放心，文件保存了")
        elif cmd=="终端输入" and dev==True:
            out_os = """注意⚠️：
                1.这是开发者模式，可能会出现各种奇怪的问题
                2.请你保证输入的指令是安全的
                3.不要瞎输入，出来问题自己承担
                4.仅限Windows7以上、macOS Big Sur以上
                5.仅能使用一次运行指令，不能是python等指令"""
            print(out_os)
            time.sleep(1)
            system = input("输入终端指令：")
            determine = input("Sicloud接收到的指令是：{0}\n是否准确(Y/N)：".format(system))
            if determine == "Y" or determine == "y":
                ...
                print("输入错误,退出")
        elif cmd=="thisisunsafe":
            verify_key={"AirTouch0914":"20110914lmb","時の旅人":"Time_Traveler"}
            invitation_code=("45R6G29Y","26S9R4JQ","358H3L26","25W4U642","0T98KP79","3O5I6M7U","1A6E4Q56","L3SK85EA","5W23Q4A5","A2AF4S4I","E8T64F5L",)
            login_mode=input("username or invitation code to log in 使用用户名登录/邀请码登录:")
            if login_mode=="username" or login_mode=="用户名":
                verify_username=input("verify username:")
                print("-----------verify username-----------")
                time.sleep(0.5)
                if verify_username in verify_key.keys():
                    print("done! input password 输入密码")
                    verify_password = input("verify password:")
                    print("-----------verify password-----------")
                    time.sleep(0.5)
                    if verify_password==verify_key[verify_username]:
                        print("done! welcome to Developer mode (bug mode[doge])")
                        dev=True
                        out="""工具箱指令：
    [help]查看所有工具箱指令：查看工具箱版本
    [加密机]安全加密
    ...
    工具箱版本：
        Sicloud 4
                      """
                        ...
                        print("password wrong! back! 密码错误！返回！")
                else:
                    print("username wrong! back! 用户名错误！返回！")
            elif login_mode=="invitation code" or login_mode=="邀请码":
                verify_code=input("invitation code:")
                if verify_code in invitation_code:
                    print("done! welcome to Developer mode")
                else:
                    print("invitation code wrong! \n 开发者模式现已开启内测，前往https://wj.qq.com/s2/12297257/334d/获取内测资格")
            else:
                print("input error！back！ 输入有误！返回！")
